...
from ..utils.submit_utils import load_cfg_from_fold_dir, best_ckpt_path, safe_torch_load
from ..utils.metrics import update_confusion_matrix, compute_metrics


def run_infer(exp_dir: str, fold: int, save_dir: str = None, batch_mul: int = 1, 
                      no_books_protect: bool = False, split_mode: str = None, save_logits: bool = True, limit: int = 0):
    
    fold_dir = os.path.join(exp_dir, f"fold{fold}")
    if not os.path.exists(fold_dir):
        # Maybe exp_dir refers to output/exp_name (which is standard)
        # Check if exp_dir has folds
        pass
    ...
    if limit > 0:
        ...
        print(f"!!! LIMITING VALIDATION DATA TO {limit} IMAGES !!!")
    ...
    loader = DataLoader(
        ds, 
        batch_size=max(1, int(cfg.BATCH_SIZE) * max(1, batch_mul)),
        shuffle=False, 

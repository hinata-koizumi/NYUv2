...
class Predictor:
    ...
    def _unpad_and_resize_logits(self, logits_chw: torch.Tensor, meta_item: dict) -> torch.Tensor:
        ...
        logits_crop = logits_chw[:, :valid_h, :valid_w]

        x = logits_crop.unsqueeze(0)
        x = F.interpolate(x, size=(orig_h, orig_w), mode="bilinear", align_corners=False)
        return x.squeeze(0)

    @torch.no_grad()
    def predict_logits(self, *, tta_combs=None, temperature: float = 1.0) -> Generator[np.ndarray, None, None]:
        ...
        for batch in tqdm(self.loader, desc="Inference", disable=(not self.verbose)):
            ...
            for i in range(bsz):
                ...
                probs_i = self._unpad_and_resize_logits(probs_cpu[i], meta_i)

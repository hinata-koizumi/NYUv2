...
def train_one_epoch(model, loader, optimizer, scaler, loss_fn, epoch, device):
    ...
    for batch in pbar:
        ...
        if len(losses) == 0 and planar_target is not None and planar_valid is not None:
            valid_sum = float(planar_valid.sum().item())
            if valid_sum > 0:
                true_rate = float((planar_target * planar_valid).sum().item()) / valid_sum
            else:
                true_rate = 0.0
            print(f"Planar target true%: {true_rate * 100:.1f}%")
...
def main():
    ...
    for ep in range(args.epochs):
        ...
        log_str += f" | gBooks/Table: {metrics.get('giou_books',0):.3f}/{metrics.get('giou_table',0):.3f}"
        print(log_str)
        print(
            "Books tp/union/pred/gt: "
            f"{metrics.get('tp_books',0):.0f}/"
            f"{metrics.get('union_books',0):.0f}/"
            f"{metrics.get('pred_books_pixels',0):.0f}/"
            f"{metrics.get('gt_books_pixels',0):.0f}"
        )
        print(
            "Table tp/union/pred/gt: "
            f"{metrics.get('tp_table',0):.0f}/"
            f"{metrics.get('union_table',0):.0f}/"
            f"{metrics.get('pred_table_pixels',0):.0f}/"
            f"{metrics.get('gt_table_pixels',0):.0f}"
        )

        # Safety stops disabled for full run

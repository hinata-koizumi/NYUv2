...
class NYUDataset(Dataset):
    ...
    def _smart_crop_wrapper(self, img, lbl, depth_m, valid, crop_h, crop_w):
        # [NEW] Safe Confusion Focus: 5% chance to focus on Sofa(3)/Bed(4)
        # Warmup: Only active after Epoch 5 to prevent early collapse.
        curr_epoch = getattr(self.cfg, "CURRENT_EPOCH", 0)
        if curr_epoch > 5 and np.random.rand() < 0.05:
             return smart_crop(
    ...
    def __getitem__(self, idx: int):
        ...
        if self.label_paths is not None:
            ...
            # NYUv2 labels are ALREADY 0-indexed (0-12) + ignore (255)
        ...
        # Depth target for auxiliary loss (normalized linear depth, masked)
        depth_target = None
        depth_valid_t = None
        if bool(getattr(self.cfg, "USE_DEPTH_AUX", False)) and float(getattr(self.cfg, "DEPTH_LOSS_LAMBDA", 0.0)) > 0.0:
            denom = float(self.cfg.DEPTH_MAX - self.cfg.DEPTH_MIN) if float(self.cfg.DEPTH_MAX - self.cfg.DEPTH_MIN) != 0 else 1.0
            depth_target_np = ((depth_m - float(self.cfg.DEPTH_MIN)) / denom).astype(np.float32)
            # mask invalid to 0 (so loss can ignore using depth_valid)
            depth_target_np = depth_target_np * valid_mask.astype(np.float32)
            depth_target = torch.from_numpy(depth_target_np).unsqueeze(0).float()
            depth_valid_t = torch.from_numpy(valid_mask.astype(np.float32)).unsqueeze(0).float()

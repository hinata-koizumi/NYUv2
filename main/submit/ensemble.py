"""
Fold ensemble inference (fixed recipe) -> submission.npy (+ submission.zip).

Fixed recipe:
  - per-fold: model_best.pth only
  - global temperature: read from OOF summary (or overridden by --temp)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="Fold ensemble inference for an existing experiment directory (fixed recipe).",
    )
    p.add_argument("--exp_dir", type=str, required=True)
    p.add_argument("--out_dir", type=str, default=None)
    p.add_argument("--folds", type=str, default=None, help="Comma-separated fold indices (default: auto-discover).")
    p.add_argument("--temp", type=float, default=None, help="Override global temperature and skip reading oof_summary.")
    p.add_argument("--oof_summary", type=str, default=None, help="Path to oof_summary.json (default: <exp_dir>/oof_summary.json).")
    p.add_argument("--data_root", type=str, default=None, help="Override cfg.DATA_ROOT.")
    p.add_argument("--batch_mul", type=int, default=2, help="Batch size multiplier relative to cfg.BATCH_SIZE.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        for f in folds:
            ...
            # Avoid any external pretrained-weight download at submit-time.
            # The checkpoint contains the full encoder weights.
        ...
        # Always clean up the large memmap file to avoid filling disk over multiple runs.
        try:
            if acc is not None:
                try:
                    acc.flush()

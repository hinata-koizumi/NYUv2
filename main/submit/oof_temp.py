"""
Compute a *global* temperature T* via OOF (out-of-fold) evaluation (fixed recipe).

Fixed recipe:
  - per-fold: model_best.pth only
  - fixed TTA: scales [0.75, 1.0] × hflip {False, True}
  - fold split: reconstructed from cfg.SEED / cfg.N_FOLDS (KFold) on sorted train keys
  - aggregate confusion matrices across folds, choose T that maximizes OOF mIoU
"""
...
def main(argv: list[str] | None = None) -> None:
    ...
    def _tta_probs(model, x: torch.Tensor, temperature: float) -> torch.Tensor:
        ...
        for scale, hflip in tta_combs:
            x_aug = x
            if float(scale) != 1.0:
                nh = max(1, int(round(base_h * float(scale))))
                nw = max(1, int(round(base_w * float(scale))))
    ...
    for fold_idx in folds:
        ...
        # Avoid any external pretrained-weight download at eval-time.

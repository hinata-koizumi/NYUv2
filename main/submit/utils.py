"""
Internal utilities for the submit pipeline (fixed recipe).

Used by:
  - main/submit/oof_temp.py
  - main/submit/ensemble.py
  - main/submit/run.py
"""
...
def load_cfg_from_fold_dir(fold_dir: str):
    ...
    `config_resolved.json` also contains derived fields (TRAIN_DIR/TEST_DIR/DEVICE),
    so we filter to actual dataclass fields.
    """
...
def best_ckpt_path(fold_dir: str) -> str:
    """Fixed recipe: best ckpt is `model_best.pth` only."""
...
def fixed_tta_combs() -> list[tuple[float, bool]]:
    """
    Fixed TTA recipe shared across all folds:
      - scales: [0.75, 1.0]
      - hflip: [False, True]
    """
    scales = [1.0, 1.25, 1.5]
...
def safe_torch_load(path: str, *, map_location: str = "cpu"):
    """
    Load a state_dict from disk.

    On newer PyTorch, prefer `weights_only=True` to avoid pickle execution and
    suppress the FutureWarning. Fall back for older versions.
    """
    import torch

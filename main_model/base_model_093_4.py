...
class Config:
    ...
    # 0: Wall, 1: Floor, 2: Cabinet, 3: Bed, 4: Chair, 5: Sofa, 6: Table, 7: Door, 8: Window, 9: Bookshelf, 10: Picture, 11: Counter, 12: Blinds? 
    # Wait, checking actual IDs or assuming explicit request?
    # User said: "Books (1), Table (9), TV (10)". 
    # BUT wait, common NYUv2 13-class mapping might differ.
    # In base_model_093_3.py SMALL_OBJ_IDS = [1, 3, 6, 7, 10].
    # Usually: 0=bed, 1=books, 2=ceil, 3=chair, 4=floor, 5=furn, 6=obj, 7=pict, 8=sofa, 9=tab, 10=tv, 11=wall, 12=win?
    # NO. 13-class mappings are often:
    # 0: Bed, 1: Books, 2: Ceiling, 3: Chair, 4: Floor...
    # Let's trust the User's Indices: "Books (1), Table (9), TV (10)".
    # Even if labels don't match standard names, optimizing for these IDs is the goal.
...
class BoundaryAwareCELoss(nn.Module):
    def __init__(self, num_classes, ignore_index, class_weights=None, boundary_weight=1.0):
        super().__init__()
    ...
    def forward(self, pred, target):
        loss_map = self.ce_loss(pred, target) # [B, H, W]
        
        # Clean labels for edge detection
        with torch.no_grad():
            lbl_edge = target.clone()
            # If target has ignore_index, treat as 0 or keep? 
            # Usually ignoring them in edge detection is safer to avoid fake edges
            lbl_edge[target == self.ignore_index] = 0
            
            lbl_float = lbl_edge.float().unsqueeze(1) # [B, 1, H, W]
            
            # Max Pool 3x3 for edges
            pooled_max = F.max_pool2d(lbl_float, kernel_size=3, stride=1, padding=1)
            pooled_min = -F.max_pool2d(-lbl_float, kernel_size=3, stride=1, padding=1)
            
            edge_mask = (pooled_max - pooled_min) > 0 # [B, 1, H, W]
            edge_mask = edge_mask.squeeze(1).float() # [B, H, W]
            
            # Weights: 1.0 + (W-1)*edge
            pixel_weights = torch.ones_like(target, dtype=torch.float32)
            pixel_weights = pixel_weights + (self.boundary_weight - 1.0) * edge_mask
            
        valid_mask = (target != self.ignore_index).float()
        # CE loss is already 0 at ignore_index, but pixel_weights might not be.
        # We need denominator = sum of weights on valid pixels.
        weighted_loss = (loss_map * pixel_weights).sum()
        denominator = (pixel_weights * valid_mask).sum() + 1e-6
...
# --- TTA Logic ---
def tta_inference(model, image, cfg, temperature=1.0, mean_tensor=None, std_tensor=None):
    # Convert torch tensor to numpy array if needed
    if torch.is_tensor(image):
        # image is [C, H, W] tensor with values in [0, 1]
    ...
    # Use passed tensors or create them if not provided (fallback)
    ...
    for scale, flip in cfg.TTA_COMBS:
        ...
        # Check alignment for UNet-like decoders, factors of 32 are safe
        h_new = (h_new // 32) * 32
        w_new = (w_new // 32) * 32
...
def validate_tta_sweep(model, dataset, cfg):
    ...
    # Pre-calculate mean/std tensors once
    mean_tensor = torch.tensor(cfg.MEAN).view(1, 1, 3).to(cfg.DEVICE)
    std_tensor = torch.tensor(cfg.STD).view(1, 1, 3).to(cfg.DEVICE)
    ...
    # Store per-temperature results: {temp: {'miou': val, 'cm': matrix}}
    results = {}
    
    # First, we need to run inference and collect probs relative to T=1.0 logic, 
    # but TTA combination happens inside tta_inference. 
    # Actually, tta_inference computes weighted probs. 
    # To be efficient, can we just run inference once per image per augmentation, 
    # and then apply temperatures?
    # tta_inference applies Softmax(logits / temp). 
    # Since Softmax is non-linear, we cannot easily reuse logits across temperatures 
    # unless we change tta_inference to return logits. 
    # BUT, the user request says: "validate_tta_sweep loops... then for all_fold_results loops again... 
    # validate_tta_sweepの中で tempごとの confusion_matrix も保存しておいて compute_oof_metrics ではそれを足し合わせるだけ"
    # So we just need to save the CMs here.
    ...
    for t in cfg.TEMPERATURES:
        ...
        for i in tqdm(range(len(dataset)), desc=f"TTA T={t}"):
             ...
             # Pass pre-calculated tensors
             avg_probs = tta_inference(model, image, cfg, temperature=t, mean_tensor=mean_tensor, std_tensor=std_tensor)
...
def main():
    ...
    for fold_idx in range(cfg.N_FOLDS):
        ...
        class_weights_tensor = torch.tensor(cfg.CLASS_WEIGHTS).float().to(cfg.DEVICE)
        
        criterion_seg = BoundaryAwareCELoss(
        ...
        fold_best_temp, fold_best_iou, fold_detailed_results = validate_tta_sweep(best_model, valid_dataset_tta, cfg)
        
        # Format results for simple saving (exclude CM)
        fold_temp_results = {t: res['miou'] for t, res in fold_detailed_results.items()}
        ...
        # OOF計算用に各温度でのconfusion matrixを保存 (From stored results, no re-calc!)
        fold_result = {}
        for temp in cfg.TEMPERATURES:
            fold_result[f'confusion_matrix_T{temp}'] = fold_detailed_results[temp]['cm']

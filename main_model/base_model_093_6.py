...
# --- Configuration ---
class Config:
    EXP_NAME = "exp098_deeplabv3p_convnext"
    
    # Architecture Settings
    ARCH = "DeepLabV3Plus"  # Options: "FPN", "DeepLabV3Plus"
    ENCODER = "tu-convnext_base" # Options: "tu-convnext_base", "efficientnet-b4"
...
class MultiTaskModel(nn.Module):
    def __init__(self, cfg):
        ...
        # Instantiate Backbone (Encoder + Decoder) based on ARCH
        model_class = getattr(smp, cfg.ARCH)
        ...
        # Determine Decoder Output Channels specifically
        # SMP architectures differ in where they store this info.
        # FPN: .decoder.out_channels ?? Or check segmentation head input.
        # DeepLabV3+: .decoder.out_channels ??
        # Safer methodology: Inspect the segmentation head's input channels.
        # usually backbone.segmentation_head[0] is a Conv2d.
        
        try:
            # Check segmentation head first layer
            if hasattr(self.backbone, 'segmentation_head') and self.backbone.segmentation_head is not None:
                ...
                if isinstance(first_layer, nn.Conv2d):
                    ...
                    # Fallback logic if head structure differs
                     decoder_channels = 256 # Common default for FPN/DeepLab
            else:
        ...
        # Create Depth Head
        # Note: Decoder features in SMP are generally accessible via different means.
        # For 'forward', SMP models return logits directly.
        # To get embeddings, we might need to use 'forward_image' or hooks, Or just standard forward for seg
        # AND manual call for depth? 
        # SMP models don't easily output "decoder features" in standard forward.
        # We need to sub-class or call parts manually.
        # Manual call seems safest for multi-task on shared decoder features.
        
        self.depth_head = nn.Conv2d(in_channels=decoder_channels, out_channels=1, kernel_size=3, padding=1)
    
    def forward(self, x):
        # Manual forward pass to access decoder features
        
        # 1. Encoder
        features = self.backbone.encoder(x)
        
        # 2. Decoder
        # SMP decoders accept the list of features
        decoder_output = self.backbone.decoder(*features)
        
        # 3. Heads
        seg_logits = self.backbone.segmentation_head(decoder_output)
        
        depth_pred = self.depth_head(decoder_output)
        
        # Upsample depth if needed (Decoder output matches logits resolution relative to input depending on arch)
        # DeepLabV3+ output is usually stride 4 or similar? 
        # seg_logits will be upsampled by segmentation_head usually? 
        # SMP segmentation head typically contains an UpdateSampling (if needed) or the model does?
        # Wait, SMP `segmentation_head` ends with Upsampling usually if `upsampling` param is set.
        # We didn't set `upsampling`. Default is usually no upsampling in head, it happens outside?
        # Actually SMP models usually return logits of same size as input (if possible) or stride 4.
        # Let's align depth to seg_logits spatial size.

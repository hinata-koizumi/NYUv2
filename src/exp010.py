...
class NYUDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.depth_paths is not None:
            ...
            # Resize to match IMAGE_SIZE (H, W) -> cv2.resize takes (W, H)
            depth = cv2.resize(
                depth,
        ...
        # --- Albumentations (RGB + label only) ---
        if self.transform:
            augmented = self.transform(image=image, mask=label)
            image = augmented['image']  # torch.Tensor [3,H,W]
            label = augmented['mask']   # torch.Tensor [H,W]
...
def main():
    ...
    train_dataset = NYUDataset(
        train_images, train_labels, depth_paths=train_depths,
        transform=get_transforms(cfg), cfg=cfg
    )

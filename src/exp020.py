...
# --- Configuration ---
class Config:
    EXP_NAME = "exp020_rgbd_late_geometric"
    SEED = 42
...
class NYUDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.depth_paths is not None:
            ...
            depth = (depth - dmin) / (dmax - dmin)      # [0,1] に正規化
...
# --- Model Definition (Late Fusion) ---
class LateFusionDeepLab(nn.Module):
    def __init__(self, num_classes: int):
        super().__init__()

        # RGB ブランチ: ResNet50, in_channels=3
        ...
        # Depth ブランチ: ResNet18, in_channels=1
        # （capacity 抑えめで overfit しすぎないように）
        self.depth_model = smp.DeepLabV3Plus(
            encoder_name="resnet18",
            encoder_weights=None,   # 1ch なので素のランダム初期化
            in_channels=1,
            classes=num_classes,
        )

        # ロジット結合用 1x1 conv（2C → C）
    ...
    def forward(self, x):
        ...
        logits_rgb = self.rgb_model(rgb)      # [B, C, H, W]
        logits_depth = self.depth_model(depth)  # [B, C, H, W]

        fused = torch.cat([logits_rgb, logits_depth], dim=1)  # [B, 2C, H, W]
...
def main():
    ...
    # --- Model ---
    model = LateFusionDeepLab(num_classes=cfg.NUM_CLASSES)
    model.to(cfg.DEVICE)

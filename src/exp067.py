...
class Config:
    ...
    # Fold
    N_FOLDS = 5
    FOLD = 0 # Change this to run different folds (0-4)
    ...
    # Output dir includes fold to avoid overwriting
...
def get_train_transforms(cfg):
    return A.Compose(
        [
            ...
            # Random Scale (Scaling around 1.0)
            A.ShiftScaleRotate(
                shift_limit=0.0, # Turn off shift/rotate here if we just want scale, or keep small
                scale_limit=0.2, # +/- 20%
                rotate_limit=0,
            ...
            # 3. Random Crop
            A.RandomCrop(
                height=cfg.CROP_SIZE[0],
                width=cfg.CROP_SIZE[1],
                p=1.0,
            ),
        ],
        additional_targets={
...
def get_valid_transforms(cfg):
    return A.Compose(
        [
            # Resize to Crop Size for consistency in evaluation? 
            # Or keep original? Usually better to have fixed size. 
            # Let's resize to the Training Crop Size (512x512) to be fair/consistent.
...
def main():
    ...
    # Get the indices for the specific folds
    # kf.split returns a generator, so we just iterate to find the one we want
    ...
    train_loader = DataLoader(
        ...
        drop_last=True # Good practice for BatchNorm
    )

...
def build_depth_geo(depth_tensor: torch.Tensor) -> torch.Tensor:
    """
    depth_tensor: [1, H, W] log-depth normalized to 0-1
    returns: [4, H, W] stacked depth, normalized depth, dx, dy
    """
...
class NYUv2Segmentation(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if self.use_depth:
            geo = build_depth_geo(depth_t[:1])
            x = torch.cat([rgb, geo], dim=0)
        else:
            x = rgb

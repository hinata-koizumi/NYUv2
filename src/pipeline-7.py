    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Inference"):
            if config.use_depth:
                ...
                inputs, name = batch
                rgb = inputs.to(device, non_blocking=True)
                depth = torch.zeros_like(rgb[:, :1])
                rgb_flip = torch.flip(rgb, dims=[3])
                depth_flip = torch.zeros_like(rgb_flip[:, :1])
...
def parse_args():
    parser = argparse.ArgumentParser(description="NYUv2 Seg - Experiment B2: Depth Gating / Attention Fusion")
    parser.add_argument("--dataset-root", type=Path, default=Path("data"), help="Path to data")
    parser.add_argument("--output-dir", type=Path, default=Path("data/output_B2"))

...
# --- Configuration ---
class Config:
    EXP_NAME = "exp050_rgbd_xy_inverse_resnet101"
    SEED = 42
    ...
    IN_CHANNELS = 6  # RGB(3) + Depth(1) + X(1) + Y(1)
...
class NYUDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # --- XY Coordinates Addition ---
        _, H, W = image.shape
        xs = np.linspace(0, 1, W, dtype=np.float32)
        ys = np.linspace(0, 1, H, dtype=np.float32)
        x_map, y_map = np.meshgrid(xs, ys) # [H, W]
        
        coord = np.stack([x_map, y_map], axis=0) # [2, H, W]
        coord_tensor = torch.from_numpy(coord).float()
        
        image = torch.cat([image, coord_tensor], dim=0) # [6, H, W] (3+1+2)
...
def save_visualizations(images, labels, preds, output_dir, epoch, cfg):
    ...
    for i in range(min(3, len(images))):
        ...
        # Visualizing XY could be cool, but skipped for now to keep it simple or just stack them? 
        # Let's just visualize RGB and Depth as before.

...
# --- Configuration ---
class Config:
    EXP_NAME = "exp092_unetpp_effb4_smartcrop"
    SEED = 42
...
class MultiTaskUNetPlusPlus(nn.Module):
    def __init__(self, num_classes, in_channels):
        ...
        # Determine depth head input channels.
        # Try to read dynamically, similar to FPN experiment.
        try:
             # Try inspecting segmentation head
             decoder_channels = self.backbone.segmentation_head[0].in_channels
        except (AttributeError, IndexError):
             # Fallback default for Unet++ (often 32 or 16 depending on decoder_channels arg)
             # Default decoder_channels for Unet++ is (256, 128, 64, 32, 16)
             # So output is 16.
             decoder_channels = 16
    ...
    def forward(self, x):
        ...
        # Upsample depth if needed (Unet++ decoder output is usually same size as input if efficientnet stride is handled, 
        # but let's check. EfficientNet reductions are usually 32x.
        # Unet++ decoder usually restores full resolution if configured so?
        # Actually smp.UnetPlusPlus decoder output stride is 1 by default? No, it depends.
        # Let's inspect in verification. For safety, we keep the Interpolate.
...
def main():
    ...
    model = MultiTaskUNetPlusPlus(num_classes=cfg.NUM_CLASSES, in_channels=4)
    model.to(cfg.DEVICE)
